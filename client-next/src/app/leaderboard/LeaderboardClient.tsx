...
function PodiumSection({ topThree }: { topThree: LeaderboardRowData[] }) {
  const p1 = topThree[0] || { name: "Name", star: "5", rating: "Current Rating", codechefUsername: "" };
  const p2 = topThree[1] || { name: "Name", star: "4", rating: "Current Rating", codechefUsername: "" };
  const p3 = topThree[2] || { name: "Name", star: "3", rating: "Current Rating", codechefUsername: "" };
...
function LeaderboardRow({
  ...
  return (
    ...
    ">
      ...
      {/* Chef column */}
      <div className="min-w-0 w-full sm:w-[45%] text-left flex items-center gap-2 sm:gap-3">
      ...
      <div className="w-auto sm:w-[25%] flex justify-end sm:justify-center items-center gap-1.5 font-mono text-xs sm:text-sm uppercase tracking-wide">
        <span>{rating}</span>
        <span className="text-red-500 animate-pulse hidden sm:inline">🔥</span>
      </div>

      {/* Action / View Profile column */}
      <div className="col-span-3 sm:col-span-1 mt-3 sm:mt-0 w-full sm:w-[20%] flex justify-end">
        <a
          href={codechefUsername ? `https://www.codechef.com/users/${codechefUsername}` : undefined}
          target="_blank"
          rel="noopener noreferrer"
          className="w-full sm:w-auto text-center px-3.5 py-1.5 sm:py-1 rounded border-2 border-black font-sans font-bold shadow-[2px_2px_0px_rgba(0,0,0,1)] text-xs bg-[#1E3A8A] text-white hover:bg-blue-800 transition-colors"
        >
          View
        </a>
      </div>
...
function LeaderboardTable({ rows }: { rows: LeaderboardRowData[] }) {
    ...
    <section className="w-full max-w-4xl mx-auto px-4 sm:px-6 mb-24 relative z-10">
      ...
      <div className="relative bg-[#F7F0DC] border-[3px] border-black p-3 sm:p-8 rounded-lg shadow-[8px_8px_0px_rgba(0,0,0,1)] sm:shadow-[12px_12px_0px_rgba(0,0,0,1)] pt-10 border-t-white/20 border-l-white/20">
        ...
        <div className="hidden sm:flex justify-between items-center w-full bg-white border-2 border-black rounded-lg px-6 py-2.5 mb-4 font-sans font-bold text-xs sm:text-sm text-neutral-600 shadow-sm uppercase tracking-wider select-none">
          ...
          <span className="w-[45%] text-left">MEMBER</span>
          <span className="w-[25%] text-center">CURRENT RATING</span>
          <span className="w-[20%] text-right">PROFILE</span>
        </div>
...
// Main Page Component
export default function LeaderboardClient({ rows }: { rows: LeaderboardRowData[] }) {
